...
export function UsageIndicator() {
  const { user } = useAuth();
  const [usage, setUsage] = useState<{ chat: number; quiz: number; voice: number } | null>(null);

  useEffect(() => {
    if (!user) return;
    const supabase = createClient();
    supabase.from("user_usage").select("chat_count, quiz_count, voice_count").eq("user_id", user.id).maybeSingle().then(
      ({ data }) => {
        if (data) setUsage({ chat: data.chat_count, quiz: data.quiz_count, voice: data.voice_count });
      }
    );
  }, [user]);
