...
export default function SidebarRight() {
  ...
  const [mOpen, setIsMobileOpen] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        for (let n = 0; n < convRows.length; n++) convIds.push(convRows[n].id);
  ...
  if (subjects.length > 0) {
    let sum = 0;
    for (let n = 0; n < subjects.length; n++) sum += subjects[n].mastery_level;
    masteryTotal = Math.round(sum / subjects.length);
  }

  const studyHours = ((messageCount * 2) / 60).toFixed(1);
  const estimatedXP = messageCount * 10 + documents.length * 25;

  const indexingDocs: { id: string; filename: string; status?: string }[] = [];
  const readyDocs: { id: string; filename: string; status?: string }[] = [];
  for (let n = 0; n < documents.length; n++) {
    const d = documents[n];
    if (d.status === "INDEXING") indexingDocs.push(d);
    else if (d.status === "READY" || !d.status) readyDocs.push(d);
  }
  ...
  const sidebarContent = (
    <>
      {loading ? (
        <>
          <div className="animate-pulse bg-white rounded-[24px] border border-[#E7E1D6] h-64 editorial" />
          <div className="animate-pulse bg-white rounded-[24px] border border-[#E7E1D6] h-28 editorial" />
          <div className="animate-pulse bg-white rounded-[24px] border border-[#E7E1D6] h-28 editorial" />
        </>
      ) : (
        <div className="flex flex-col h-full space-y-10">
              ...
              <div className="space-y-3">
                {memories.map((m, i) => (
                  <div key={i} className="bg-white rounded-[32px] p-5 border border-[#E7E1D6] shadow-[0_10px_30px_-5px_rgba(0,0,0,0.05)] flex items-center gap-4 editorial">
                    <div className="w-11 h-11 shrink-0 rounded-2xl flex items-center justify-center editorial" style={{ backgroundColor: "#E8F1E6" }}>
            ...
            <div>
              ...
              <div className="space-y-3">
                {indexingDocs.slice(0, 2).map((doc) => (
                  <div key={doc.id} className="bg-white rounded-[32px] p-5 border border-[#E7E1D6] shadow-[0_10px_30px_-5px_rgba(0,0,0,0.05)] flex items-center gap-3 editorial">
                    <div className="w-11 h-11 shrink-0 rounded-2xl flex items-center justify-center editorial" style={{ backgroundColor: "#E1EAF4" }}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="w-full h-1 bg-[#E7E1D6] rounded-full mt-2 overflow-hidden">
          ...
          <div className="flex-1 flex flex-col min-h-[220px]">
            {displayTrack ? (
              <div className="bg-white rounded-[40px] p-8 border border-[#E7E1D6] shadow-[0_10px_30px_-5px_rgba(0,0,0,0.05)] flex-1 flex flex-col justify-center editorial">
                ...
                <div className="flex items-center justify-center gap-6">
                  ...
                  <button
                    ...
                    style={{ backgroundColor: SAGE, boxShadow: "0 10px 24px rgba(63,92,58,0.35)" }}
                  >
                    {isPlaying ? (
              ...
              <a
                href="/music"
                className="bg-white rounded-[40px] p-8 border border-[#E7E1D6] shadow-[0_10px_30px_-5px_rgba(0,0,0,0.05)] flex-1 flex flex-col items-center justify-center text-center hover:border-[#E7E1D6] transition-colors editorial"
              >
                ...
                <span
                  ...
                  style={{ backgroundColor: SAGE, boxShadow: "0 10px 24px rgba(63,92,58,0.25)" }}
                >
    ...
    <>
      ...
      <div
        className={[
          ...
          "border-l border-[#E7E1D6] text-[#2D3436]",
          "transform transition-transform duration-300 ease-in-out",
          "overflow-y-auto",
          mOpen ? "translate-x-0" : "translate-x-full",
        ].join(" ")}
