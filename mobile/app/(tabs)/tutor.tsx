    ...
    <View style={[styles.bubbleRow, isUser ? styles.userRow : styles.botRow]}>
      ...
      {isUser ? (
        ...
        <View style={[styles.bubble, styles.botBubble]}>
          <MarkdownText content={m.content} color="#555E61" />
        </View>
      ) : m.streaming ? (
        <View style={[styles.bubble, styles.botBubble]}>
...
export default function TutorTab() {
  ...
  const startListening = async () => {
    ...
    try { await recorder.prepareToRecordAsync(); recorder.record(); setRecording(true); setStatus('Listeningâ€¦'); }
  ...
  const stopListening = async () => {
    ...
    try {
      ...
      else setStatus('No speech heard â€" try again');
            ...
            {activeId ? (
              <>
                ...
                <Pressable style={styles.headerBtn} onPress={() => setActiveId(null)}>
                  <Icon icon={ChevronLeft} size={20} color="#8A8478" />
                </Pressable>
              </>
            ) : null}
            <Pressable style={styles.headerBtn} onPress={() => setSpeaking((s) => !s)}>
              <Icon icon={Volume2} size={20} color={speaking ? GREEN : '#BBB'} />
            </Pressable>
        ...
        {needsSession ? (
          <View style={styles.center}>
            <View style={styles.emptyCard}>
              <Icon icon={MessageSquareText} size={32} color="#BBB" />
          ...
          <ScrollView style={styles.flex} contentContainerStyle={styles.listContent} showsVerticalScrollIndicator={false}>
            ...
            {loading ? (
              ...
              <View style={styles.emptyList}>
                <Icon icon={Bot} size={40} color="#BBB" />
                ...
                <Pressable style={styles.createChatBtn} onPress={createConversation}>
                  <Icon icon={Plus} size={16} color="#FDFBF7" strokeWidth={2} />
              ...
              conversations.map((c) => (
                <View key={c.id} style={styles.convRow}>
                  <Pressable style={styles.convCard} onPress={() => setActiveId(c.id)}>
                    ...
                    <Icon icon={ChevronRight} size={16} color="#BBB" />
                  </Pressable>
                  <Pressable style={styles.deleteBtn} onPress={() => deleteConversation(c.id)} hitSlop={8}>
                    <Icon icon={Trash2} size={14} color="#BBB" />
                  </Pressable>
                </View>
              ))
            )}
          ...
          <>
            ...
            >
              {messages.length === 0 && !sending ? (
                <View style={styles.center}>
                  <Icon icon={Bot} size={44} color="#BBB" />
            ...
            <View style={[styles.composerWrap, { paddingBottom: insets.bottom + 90 }]}>
              <View style={styles.composer}>
                ...
                <Pressable
                  ...
                  disabled={sending}
                >
                  {recording ? <ActivityIndicator color="#FDFBF7" size="small" /> : <Icon icon={Mic} size={18} color={GREEN} strokeWidth={2} />}
                </Pressable>
                <TextInput
                  style={styles.input}
                ...
                >
                  {sending ? <ActivityIndicator color="#FDFBF7" size="small" /> : <Icon icon={Send} size={18} color="#FDFBF7" strokeWidth={2} />}
                </Pressable>
              </View>
            </View>
          </>
...
const styles = StyleSheet.create({
  ...
  title: { fontFamily: SERIF, fontSize: 26, fontWeight: '700', color: '#2D3436' },
  subtitle: { fontFamily: SERIF, fontSize: 12, color: '#8A8478' },
  headerBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#F3EDE3', borderWidth: 2, borderColor: '#2D3436', alignItems: 'center', justifyContent: 'center' },
  ...
  emptyTitle: { fontFamily: SERIF, fontSize: 16, fontWeight: '700', color: '#2D3436', marginTop: 12, textAlign: 'center' },
  emptyDesc: { fontFamily: SERIF, fontSize: 14, color: '#555E61', textAlign: 'center', lineHeight: 20, marginTop: 6 },

  listContent: { paddingHorizontal: 24, paddingBottom: 100 },
  sectionLabel: { fontFamily: SERIF, fontSize: 12, fontWeight: '600', color: '#555E61', textTransform: 'uppercase', letterSpacing: 1.2, marginBottom: 12, marginTop: 8 },
  emptyList: { alignItems: 'center', paddingVertical: 40 },
  createChatBtn: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: GREEN, borderWidth: 2, borderColor: '#2D3436', borderRadius: 255, borderTopLeftRadius: 255, borderTopRightRadius: 15, borderBottomLeftRadius: 225, borderBottomRightRadius: 15, paddingHorizontal: 20, paddingVertical: 12, marginTop: 16, boxShadow: '3px 3px 0 0 #2D3436' },
  createChatBtnText: { color: '#FDFBF7', fontFamily: SERIF, fontWeight: '700', fontSize: 14, textTransform: 'uppercase' },
  convRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 10 },
  convCard: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: '#FDFBF7', borderRadius: 20, borderTopLeftRadius: 20, borderTopRightRadius: 8, borderBottomLeftRadius: 22, borderBottomRightRadius: 7, borderWidth: 2, borderColor: '#2D3436', paddingHorizontal: 16, paddingVertical: 14, boxShadow: '3px 3px 0 0 #2D3436' },
  ...
  convTitle: { fontFamily: SERIF, fontSize: 14, fontWeight: '600', color: '#2D3436' },
  convDate: { fontFamily: SERIF, fontSize: 11, color: '#8A8478' },
  deleteBtn: { padding: 8 },
  ...
  avatar: { width: 30, height: 30, borderRadius: 15, backgroundColor: '#E8F0E5', borderWidth: 2, borderColor: '#2D3436', alignItems: 'center', justifyContent: 'center', marginBottom: 4 },
  bubble: { maxWidth: '80%', paddingHorizontal: 16, paddingVertical: 12, borderRadius: 20 },
  userBubble: { backgroundColor: GREEN, borderBottomRightRadius: 6, borderWidth: 2, borderColor: '#2D3436' },
  botBubble: { backgroundColor: '#FDFBF7', borderBottomLeftRadius: 6, borderWidth: 2, borderColor: '#2D3436', boxShadow: '3px 3px 0 0 #2D3436' },
  userBubbleText: { color: '#FDFBF7', fontFamily: SERIF, fontSize: 15, lineHeight: 22 },
  statusRow: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 4 },
  statusText: { fontFamily: SERIF, fontSize: 12, color: '#8A8478', flexShrink: 1 },

  composerWrap: { paddingHorizontal: 24, paddingTop: 8 },
  composer: { flexDirection: 'row', alignItems: 'flex-end', backgroundColor: '#FDFBF7', borderRadius: 20, borderTopLeftRadius: 20, borderTopRightRadius: 8, borderBottomLeftRadius: 22, borderBottomRightRadius: 7, borderWidth: 2, borderColor: '#2D3436', paddingVertical: 6, paddingLeft: 8, paddingRight: 6, gap: 6, boxShadow: '3px 3px 0 0 #2D3436' },
  attachBtn: { width: 38, height: 38, borderRadius: 12, backgroundColor: '#E8F0E5', borderWidth: 2, borderColor: '#2D3436', alignItems: 'center', justifyContent: 'center' },
  micBtn: { width: 38, height: 38, borderRadius: 12, borderWidth: 2, borderColor: '#2D3436', alignItems: 'center', justifyContent: 'center' },
  input: { flex: 1, color: '#2D3436', fontFamily: SERIF, fontSize: 15, maxHeight: 120, paddingTop: 8, paddingBottom: 8 },
  sendBtn: { width: 38, height: 38, borderRadius: 19, backgroundColor: GREEN, borderWidth: 2, borderColor: '#2D3436', alignItems: 'center', justifyContent: 'center' },
