    ...
    <div className="min-h-screen bg-deep-onyx text-white flex flex-col">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              <span className="text-[9px] font-bold uppercase text-white/20 px-1.5 py-0.5 rounded-full bg-white/5">
                {challenge.language}
              </span>
            </div>
          </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-3xl mx-auto p-6 space-y-6">
            ...
            {cells.map((code, i) => (
              <div key={i}>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[10px] font-bold text-white/20">Cell {i + 1}</span>
                  {cells.length > 1 && (
                    <button
                      onClick={() => {
          ...
          <button
            onClick={addCell}
            className="w-full py-3 rounded-2xl border-2 border-dashed border-white/[0.06] text-xs font-bold text-white/20 hover:text-white/40 hover:border-white/10 transition-all cursor-pointer"
          >
            + Add Cell
          </button>
          ...
          {challenge.testCases && challenge.testCases.length > 0 && (
            <div className="glass-card rounded-3xl p-5">
              <h3 className="text-xs font-bold uppercase tracking-widest text-white/30 mb-3">Test Cases</h3>
              <div className="space-y-2">
                {challenge.testCases.map((tc, i) => (
                  <div key={i} className="text-xs font-mono bg-black/40 rounded-xl px-4 py-2">
                    ...
                    <span className="text-cyber-yellow/80">{tc.expected}</span>
                  </div>
                ))}
              </div>
            </div>
