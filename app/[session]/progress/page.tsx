...
export default function SessionProgressPage() {
  ...
  const vals: number[] = [];
  for (const d of ["MON", "TUE", "WED", "THU", "FRI", "SAT", "SUN"]) vals.push(trendByDay[d]);
  const maxV = Math.max(...vals, 1);
  const pts: { x: number; y: number }[] = [];
  for (let i = 0; i < vals.length; i++) {
    pts.push({ x: (i / 6) * 400, y: 140 - (vals[i] / maxV) * 120 });
  }
      ...
      `}</style>
        <div className={`${expanded ? "min-h-[40vh] p-4 sm:p-6 lg:p-12" : "h-[16vh] px-4 sm:px-6 lg:px-12 py-5"} bg-[#FDFBF7] text-[#2D3436] relative flex flex-col justify-end overflow-hidden transition-all duration-[800ms] ease-[cubic-bezier(0.22,1,0.36,1)]`}>
          <div className="absolute top-4 right-4 lg:top-10 lg:right-10 flex gap-2 lg:gap-4">
            <div className="btn-editorial px-4 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest">WEEKLY REPORT</div>
        ...
        <div className="flex-1 px-4 sm:px-6 lg:px-12 pt-6 pb-20 space-y-8 overflow-y-auto">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6 items-stretch">
            <div className="col-span-4 bg-white rounded-[32px] p-4 sm:p-6 lg:p-8 flex flex-col items-center justify-center text-center editorial">
              ...
              <div className="relative w-48 h-48 mx-auto flex items-center justify-center mb-6">
                <svg className="w-full h-full -rotate-90">
                  ...
                  <circle cx="96" cy="96" r="88" fill="transparent" stroke="#3F5C3A" strokeWidth="12" strokeDasharray="552" strokeDashoffset={masteryDashoffset} className="transition-all duration-1000 ease-out" />
                </svg>
              ...
              <div className="space-y-4 w-full">
                <div className="flex justify-between items-center bg-warm-ink/[0.04] p-3 rounded-2xl editorial">
            ...
            <div className="col-span-8 bg-white rounded-[32px] p-4 sm:p-6 lg:p-8 editorial">
              ...
              <div className="h-48 flex items-end justify-between px-4 mb-4 gap-4">
                {BAR_DATA.map((bar) => (
                  <div key={bar.day} className="flex-1 flex flex-col items-center gap-2">
                    <div
                      className={`w-full rounded-t-lg chart-bar ${bar.peak ? "bg-gradient-to-t from-sage to-cyan-400 " : "bg-warm-ink/[0.04]"}`}
                ...
                <svg viewBox="0 0 400 150" className="w-full h-full">
                  <defs>
                    <linearGradient id="fillGrad" x1="0" x2="0" y1="0" y2="1">
                      <stop offset="0%" stopColor="#3F5C3A" stopOpacity="0.3" />
                      <stop offset="100%" stopColor="#3F5C3A" stopOpacity="0" />
                    </linearGradient>
                  </defs>
                  <path d={curvePath} fill="none" stroke="#3F5C3A" strokeWidth="4" className="progress-line-animate" />
                  <path d={fillPath} fill="url(#fillGrad)" />
                  {pts.length > 0 && (() => {
                    const peakIdx = vals.indexOf(Math.max(...vals));
                    return <circle cx={pts[peakIdx].x} cy={pts[peakIdx].y} r="6" fill="#3F5C3A" className="pulse-soft" />;
                  })()}
                </svg>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {milestones.length > 0 ? (
                milestones.map((m, i) => (
                  <div key={i} className="bg-white p-5 rounded-[28px] hover:scale-105 transition-all cursor-pointer group editorial">
                    <div className="w-10 h-10 rounded-xl bg-sage/10 flex items-center justify-center mb-4 group-hover:bg-sage group-hover:text-white transition-colors editorial">
                ...
                <>
                  <div className="bg-white p-5 rounded-[28px] opacity-40 editorial">
                    <div className="w-10 h-10 rounded-xl bg-warm-ink/[0.04] flex items-center justify-center mb-4 editorial">
                      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                        <path d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                    </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
              {weaknesses.length > 0 && (
                <div className="bg-white p-4 sm:p-6 rounded-[32px] border-t border-hairline-warm hover:border-sage/50 transition-all group editorial">
                  <div className="flex justify-between items-start mb-4">
                    <div className="w-10 h-10 rounded-xl bg-warm-ink/[0.04] flex items-center justify-center editorial">
                      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                  ...
                  <a href="#" className="inline-block px-6 py-2 btn-editorial text-[10px] font-bold rounded-full uppercase tracking-tighter">Start Quiz</a>
                </div>
              )}
              {strengths.length > 0 && (
                <div className="bg-white p-4 sm:p-6 rounded-[32px] border-t border-hairline-warm hover:border-sage/50 transition-all group editorial">
              ...
              {nextModuleName && (
              <div className="bg-white p-4 sm:p-6 rounded-[32px] border-t border-hairline-warm hover:border-cyan-400/50 transition-all group editorial">
                <div className="flex justify-between items-start mb-4">
                  <div className="w-10 h-10 rounded-xl bg-warm-ink/[0.04] flex items-center justify-center editorial">
                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
